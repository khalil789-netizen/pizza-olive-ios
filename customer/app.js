...
// --- Customer shim: expose rolled-manoushe check on window (used by some click handlers)
window.isRolledManoushe = window.isRolledManoushe || function(item){
  try{
    const normKey = (v)=> (v==null?"":String(v)).toLowerCase().trim();
    const name = normKey((item && (item.nameHe||item.nameAr||item.nameEn||item.name||item.title||"")));
    const cat  = normKey((item && (item.category||item.cat||item.categoryId||"")));
    const rolled = (
      name.includes("מגולג") || name.includes("מגוגל") || name.includes("roll") || name.includes("rolled") ||
      name.includes("ملف") || name.includes("رول")
    );
    const isMan = (cat.includes("manous") || cat.includes("mana") || cat.includes("منق") || cat.includes("מנא"));
    return rolled && (isMan || rolled);
  } catch(e){
    return false;
  }
};
...
if(!pending) return;
  ...
  // Keep everything inside the visible pizza circle (avoid edges)
  const baseR = Math.min(w,h)/2;
  // Piece size scales with pizza size
  const uniformSize = Math.max(30, Math.round(Math.min(w,h) * 0.115));
  // Strong crust safety margin (user asked: never on the edge)
  // We also subtract half the piece size so even large pieces never touch the crust.
  // Bigger gap from the crust so pieces never sit on the edge.
  const crustGap = Math.max(10, Math.round(uniformSize * 0.25));
  const safeR = Math.max(30, baseR - crustGap - Math.round(uniformSize/2));
...
// Manoushe rolled / "magulgal" should NOT allow toppings overlay selection (per business rule)
window.isRolledManoushe = function(item){
  const name = normKey(item?.nameHe || item?.nameAr || item?.nameEn || item?.name || "");
  const cat  = normKey(item?.category || "");
  // Trigger words (Heb/Eng/Ar) for rolled items
  const rolled = (
    name.includes("מגולג") || name.includes("מגוגל") || name.includes("roll") || name.includes("rolled") ||
    name.includes("ملف") || name.includes("رول")
  );
  const isMan = (cat.includes("manous") || cat.includes("mana") || cat.includes("منق") || cat.includes("מנא"));
  return rolled && (isMan || rolled); // if it's rolled by name, we block regardless
}
...
function isPastry(item){
  ...
  if(!c) return false;
  if(c.includes("sfiha") || c.includes("sficha") || c.includes("sfi7") || c.includes("manous") || c.includes("mana2")) return true;
  if(c.includes("malaw") || c.includes("malawach") || c.includes("malawe") || c.includes("bourek") || c.includes("burek") || c.includes("pastry")) return true;
  if(c.includes("מאפה") || c.includes("מלאוח") || c.includes("בורקס") || c.includes("ספיחה") || c.includes("מנאקיש")) return true;
  if(c.includes("معجن") || c.includes("مناقي") || c.includes("سفي") || c.includes("فطا")) return true;
  return false;
}
...
function openItemModal(item){
  ...
  const _pizzaLike = (isPizza(item) || (isPizzaLike(item) && !window.isRolledManoushe(item)));
  const _catKey = normKey(item?.category);
  const _isSfiha = _catKey.includes("sfiha") || _catKey.includes("sficha") || _catKey.includes("ספיח") || _catKey.includes("سفي");
  const _isMan   = _catKey.includes("manous") || _catKey.includes("منا") || _catKey.includes("منق") || _catKey.includes("מנא");
  $("#modalTitle").textContent = _pizzaLike
    ? (_isSfiha ? "תוספות וירקות לספיחה" : (_isMan ? "תוספות וירקות למנאושה" : "תוספות וירקות לפיצה"))
    : (item.nameHe || item.nameAr || "פריט");
  $("#modalImg").src = cardImgFor(item);
  $("#modalImg").alt = item.nameHe || item.nameAr || "";

  // Make sfiha/manoushe preview as big as pizza so toppings align visually
  const _imgEl = $("#modalImg");
  if(_imgEl){
    const _isPastryLike = (_isSfiha || _isMan);
    _imgEl.classList.toggle("is-sfiha", _isSfiha);
    _imgEl.classList.toggle("is-manoushe", _isMan);
    _imgEl.classList.toggle("is-pizza", !_isPastryLike);
    _imgEl.classList.toggle("is-pastry", false);
  }
  ...
  if(isPizza(item) || (isPizzaLike(item) && !window.isRolledManoushe(item))){
  ...
  } else if(isPastryToppable(item) && !(isPizzaLike(item) && !window.isRolledManoushe(item))) {
    pending.config = { toppings: {}, typeCrust:'reg' };
    body.appendChild(buildPastryEditor(pending));
    ...
    for(const t of arr){
      ...
      btn.addEventListener("click", ()=>{
        if(isOn()){
          ...
          pending.config.toppings[t.id] = { mode:"all", quarters:[] }; // fixed ALL
        }
        refresh();
        try{ updateModalPrice(pending); }catch(e){}
      });
